$(document).ready(function () {
    $('#experience_section').on('click', '.current_work_chbox', function (e) {
        ...
        if(document.getElementById('current_work_'+id).checked){
            $(e.target).parent('div').parent('div').find('#to_period_experience_'+id).attr({'disabled': 'disabled', 'type': 'text', 'required':false});
            ...
            $(e.target).parent('div').parent('div').find('#to_period_experience_'+id).attr({'disabled':false, 'type':'date', 'required':true});
